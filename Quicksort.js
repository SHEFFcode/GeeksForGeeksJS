class Quicksort {
  QuicksortWrapper (A, start, end) {
    if (start < end) {
      var partitionIndex = this.Partition(A, start, end); // we partition the array such that elements smaller are on the right and elements higher are on the left, and return pi
      this.QuicksortWrapper(A, start, partitionIndex - 1); // Run recursion on the left side of the array
      this.QuicksortWrapper(A, partitionIndex + 1, end); // Run recursion on the right side of the array
    }
  }
